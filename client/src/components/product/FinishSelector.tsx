...
import { CheckIcon, FinishIcon } from '../icons';

const FINISH_INFO: Record<string, { desc: string }> = {
  Matte: { desc: 'No glare, elegant finish' },
  Glossy: { desc: 'Vivid colors, shine' },
};
  ...
  return (
    <div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        {finishes.map((f) => {
          const info = FINISH_INFO[f] ?? { desc: f };
          ...
          return (
            ...
            >
              <FinishIcon
                finish={f}
                size="lg"
                className={active ? 'text-white' : 'text-brand-maroon'}
              />
